...
function drawMap() {
    ...
    // Center Coordinates (ca. Paris), max. zoom and bounds for dragging:
    home_lat = 50;
    home_long = 2;
    home_zoom = 4;
    max_zoom = 20;
...
function drawPoints() {
    ...
    // Add tooltips and Infobox-Interacitivty:
    init_tooltips();
    on_click_infobox();

    // Function to update positions when zooming:
    function zoom_update() {
        ...
        var x_range = d3.extent(circle_list.map(function (c) {
            return c.cx.baseVal.value;
        }));
        var y_range = d3.extent(circle_list.map(function (c) {
            return c.cy.baseVal.value;
        }));
        var margin = 150;
...
// SELECTED CIRCLE & TOOLTIPS Interactivity:
function init_tooltips() {
    ...
    circles.on("mouseover", function (d) {
        ...
        // change strokewith of selected circle
        d3.select(this).style("stroke-width", 2);

        // Tooltips: Define the div for the tooltip
        var div = d3.select("body")
            .append("div")
            .attr("class", "tooltip");

        // write out info in a tooltip box that is placed in top-right from dot
    ...
    // decrease stroke width of selected circle and remove tooltip again:
...
function on_click_infobox() {
    ...
    circles.on("click", function (d) {
        ...
        // Write info of selected point in infobox
...
function init_legend() {
    ...
    // create the legend text for the circle sizes:
    svg.selectAll("text")
        .data(circles)
        .enter()
        .append("text")
